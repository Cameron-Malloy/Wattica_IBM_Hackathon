...
const DashboardPage = () => {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: surveyData.filter(s => s.ai_recommendation).length,
      icon: HeartIcon,
      color: 'text-green-600',
      bgColor: 'bg-green-100',
      description: 'Community-driven plans',
    ...
    <>
        ...
        {stats.map((stat, index) => {
          ...
          return (
            <motion.div
              ...
              className={`bg-white rounded-xl shadow-lg border border-white/20 p-6 card-hover ${stat.onClick ? 'cursor-pointer hover:shadow-xl transition-all duration-300' : ''}`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-gray-900 flex items-center">
              <FlagIcon className="h-6 w-6 text-orange-600 mr-2" />
              Top Priority Areas
            </h2>
            <button
              onClick={() => setActiveTab('priorities')}
          ...
          {results?.priority_areas && results.priority_areas.length > 0 ? (
            <div className="space-y-3">
              {results.priority_areas.slice(0, 3).map((priority) => (
                <div
                  key={priority.id}
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900 text-sm">{priority.location}</h3>
                      <p className="text-xs text-gray-600 mb-1">{priority.top_issue}</p>
            ...
            <div className="text-center py-4">
              ...
              <p className="text-gray-600 text-sm">No priority areas identified</p>
            </div>
          )}
        </motion.div>
      </div>
